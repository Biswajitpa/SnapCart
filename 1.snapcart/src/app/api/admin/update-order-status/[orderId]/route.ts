...
// 🎯 Normalize incoming status strings so "Out For Delivery", "out_of_delivery",
// "OUT OF DELIVERY", etc. all match the same trigger condition.
function isOutForDeliveryStatus(status: string) {
    if (!status) return false;
    const normalized = status.toLowerCase().replace(/[_-]/g, " ").trim();
    return (
        normalized === "out of delivery" ||
...
export async function POST(req: NextRequest, context: { params: Promise<{ orderId: string; }>; }) {
    try {
        ...
        if (isOutForDeliveryStatus(status) && !order.assignment) {
